...
class PyLyrics3(object):
    ...
    def __init__(self, proxies=None):
        ...
        self.artists = ['jayz', 'kendrick lamar', 'drake', '2pac', 'the_notorious_b.i.g.',
        '50 cent', 'future', 'trey songz', '2_chainz', 'big sean', 'lil wayne', 'nas',
        'dr._dre','snoop dogg','chance_the_rapper', 'kanye west','j._cole', 'ice cube', 'sean combs',
        'migos','method man','outkast', 'rakim', 'busta rhymes','redman','andré_3000','rick ross',
        'young thug','the game','wu-tang_clan','aesop rock', 'dmx', 'wiz khalifa']
        self.artist_idx = 31 #dmx
        with open('checkpoint.json') as data_file:
            checkpt = json.load(data_file)
            self.last_artist = checkpt['last_artist']
            self.last_idx = int(checkpt['last_idx'])
        try:
            self._conn = psycopg2.connect("dbname='cs291' user='vivekpradhan' host='localhost' password='foo'")
    ...
    def get_artist_lyrics(self, artist, albums=False):
        ...
        title_to_lyrics = {}
        idx = self.last_idx
        end_idx = min(idx+100,len(song_urls)-1)
        print(len(song_urls))
        while idx <= end_idx:
            url = song_urls[idx]
            #print(str(idx)+" is the index")
            ...
            if lyrics:
                tup = (artist,url,lyrics,'rap')
                #print(tup)
                self.cur.execute("INSERT INTO songlyrics (artist_name,song_name,lyrics,genre) VALUES (%s, %s, %s, %s)",tup)
                #title_to_lyrics[title] = lyrics
            if idx % 10 == 0:
                print(str(idx)+" songs fetched")
            idx = idx + 1
            #print("Index should be updated now!")
        #Close connections
        self._conn.commit()
        print("need to create checkpoint now.., idx "+str(idx))
        print("End index is "+str(end_idx))
        with open('checkpoint.json', 'w') as data_file:
            if idx >= len(song_urls)-1:
                self.artist_idx = self.artist_idx + 1
                print ("Fetched all songs for artist: "+artist)
                print ("=====================================")
                if self.artist_idx < len(self.artists):
                    artist = self.artists[self.artist_idx]
                    print ("Going to fetch songs for artist "+artist)
                else:
                    artist = ''
                    self.artist_idx = -1
                idx = 0
            data = {'last_artist':artist,'last_idx':idx}
            json.dump(data, data_file)
        print("Checkpoint created")
        time.sleep(5)
        if self.artist_idx != -1:
            self.get_artist_lyrics(artist)
        else: 
            self.cur.close()
            self._conn.close()
...
artists = __INSTANCE.artists
get_artist_lyrics(artists[__INSTANCE.artist_idx])
#while __INSTANCE.artist_index != -1:
#    get_artist_lyrics(artists[__INSTANCE.artist_index])
#jayz, kendrick lamar, drake, 2pac, the_notorious_b.i.g.