...
batch_size = 128
DATA_DIR = "/Users/vivekpradhan/Documents/cs291-DL4NLP/"
...
def get_date():
    ...
    train_data = open(DATA_DIR+"data_4/q_train.txt", "r").readlines()
    val_data = open(DATA_DIR+"data_4/q_val.txt", "r").readlines()
    style_train_data = open(DATA_DIR+"data_4/s_train.txt", "r").readlines()
    style_val_data = open(DATA_DIR+"data_4/s_val.txt", "r").readlines()
    ...
    #save
    f = open(DATA_DIR+"classifier_4/train.pkl",'wb')
    pkl.dump(( (sent_index_train, style_train_data), (sent_index_val, style_val_data) ), f )

    f = open(DATA_DIR+"classifier_4/word_index.pkl",'wb')
...
def train():
    ...
    print 'Loading data...'
    (X_train, y_train), (X_test, y_test) = pkl.load(open(DATA_DIR+"classifier_4/train.pkl",'rb'))
    ...
    open(DATA_DIR+"classifier_4/model_architecture.json",'w').write(json_string)  
    model.save_weights(DATA_DIR+"classifier_4/model_weights.h5")  

def get_test_data(test_data_path):
    file_sent = open(test_data_path,'r')
    word_index = pkl.load(open(DATA_DIR+"classifier_4/word_index.pkl",'rb'))
    sent_index = []
    while True:
...
def test(test_dir_name):
    model = model_from_json(open(DATA_DIR+"classifier_4/model_architecture.json").read())  
    model.load_weights(DATA_DIR+"classifier_4/model_weights.h5")
    

    #subdir_names = ['multi_decoder_20']
    #subdir_names = get_sub_dirnames(test_dir_name)
    #subdir_names = [i for i in subdir_names if not i.endswith("txt")]
    dir_name = 'sen4_adv_epoch5'
    #for dir_name in subdir_names:
    for index_name in ["0","1"]:
        test_x = get_test_data(test_dir_name + "/"+ dir_name +"/q_test_style"+ index_name +".txt")
        test_x = sequence.pad_sequences(test_x, maxlen=maxlen)
        print "test_x shape:"+str(test_x.shape)   
        scores=model.predict(test_x)
        f = open(test_dir_name + "/"+ dir_name +"/q_test_style"+ index_name +"_classification.txt", "w")
        for score in scores:
            f.write(str(score[0])+"\n")
        f.close()
    return
