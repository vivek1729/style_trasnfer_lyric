...
def gen_score(test_dir_name):
    ...
    emb = Embedding(100,False,dir_name) #True would load static glove embeddings
    word_dict = emb.get_all_emb()
    #subdir_names = ['multi_decoder']
    ...
    for index_name in ["0","1"]:
        q_file = test_dir_name+"/"+dir_name+"/q_test.txt"
        r_file = test_dir_name+"/"+dir_name+"/q_test_style"+index_name+".txt"
        w_file = test_dir_name+"/"+dir_name+"/q_test_style"+index_name+"_semantics.txt"
