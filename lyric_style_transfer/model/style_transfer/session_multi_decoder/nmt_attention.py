...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'gru_cond_attention':('param_init_gru_cond',
                              'gru_cond_layer')
          }
...
def build_model(tparams, options):
    ...
    embr = tparams['Wemb_encoder'][xr.flatten()]
    ...
    style_matrix0 = tensor.dot(proj[0][-1], style_matrix0)
    ...
    ctxs = proj[1][-1] #Just look at the last context

    # weights (alignment matrix)
    opt_ret['dec_alphas_0'] = proj[2]
    ...
    # weighted averages of context, generated by attention module
    ctxs = proj1[1][-1]#Just look at the last context

    # weights (alignment matrix)
    opt_ret['dec_alphas_1'] = proj1[2]
    ...
    if options['use_dropout']:
        logit1 = dropout_layer(logit, use_noise, trng)
...
def build_sampler(tparams, options, trng, use_noise, style_type):
    ...
    ctxs = proj[1][-1]#Just look at the last context again.
    ...
    if alpha_c > 0. and not model_options['decoder'].endswith('simple'):
        ...
        alpha_reg = alpha_c * (
            (tensor.cast(y_mask.sum(0)//x_mask.sum(0), 'float32')[:, None] -
             opt_ret['dec_alphas_0'].sum(0))**2).sum(1).mean()
        cost0 += alpha_reg
        cost1 += alpha_reg
